...
export default function PokemonVoteDraftBoards() {
    ...
    try {
      ...
      if (!stateRes.ok) {
        ...
        setLobby({
          ...
          competitions: stateData.competitions || ["Battle"],
        });
      }
      if (votesRes.ok) {
  ...
  const maxSelectable = useMemo(() => Math.min(3, lobby?.players.length ?? 0), [lobby?.players]);
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans relative">
      <header className="sticky top-0 z-30 bg-neutral-950/95 backdrop-blur border-b border-neutral-800">
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-white">Vote — Pokémon Draft #{lobbyId}</h1>
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          {lobby.players.map((player) => {
            ...
            return (
              <div
                key={player.id}
                className={`relative border rounded-2xl overflow-hidden bg-neutral-900/80 backdrop-blur-sm transition ${label ? col.border : "border-neutral-800"} ${disabled ? "opacity-80" : "cursor-pointer hover:border-red-500/60"}`}
              ...
              >
                <div className="p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-xs text-neutral-500">
                      BST Total: {player.popularityTotal.toLocaleString()}
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 auto-rows-fr">
                    {Object.entries(player.slots).map(([slotName, monValue]) => {
                      const mon = monValue as Pokemon | null;
                      return (
                        <div
                          key={slotName}
                        ...
                        >
                          ...
                          <div className="flex-1 bg-neutral-900 flex items-center justify-center p-1">
                            {mon ? (
                              <img
                                src={mon.image.large}
                                alt={mon.name.full}
                              ...
                              <span className="text-[11px] text-neutral-600 px-2 text-center">Empty</span>
                            )}
                          </div>
                          <div className="text-xs text-neutral-200 text-center px-1 py-1 bg-neutral-900 leading-tight">
                ...
                {label && (
                  <div className={`absolute inset-0 flex items-center justify-center text-white font-bold text-lg ${col.overlay}`}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[11px] text-neutral-500">
                          BST Total: {player?.popularityTotal.toLocaleString() ?? "0"}
                        </span>
                      </div>
                    </div>
